...
int abonado_mostrar(Abonado* array,int limite)
{
    ...
    if(limite > 0 && array != NULL)
    {
        ...
        for(i=0;i<limite;i++)
        {
            if(!array[i].isEmpty)
            {
               printf("\n[RELEASE] %s %s - CEL: %d - ID: %d - %d",array[i].nombre,array[i].apellido,array[i].numero,array[i].idAbonado,array[i].isEmpty);
            }
        }
    }
    return retorno;
...
int abonado_mostrarDebug(Abonado* array,int limite)
{
    ...
    if(limite > 0 && array != NULL)
    {
        ...
        for(i=0;i<limite;i++)
        {
            printf("\n[DEBUG] %s %s - CEL: %d - ID: %d - %d",array[i].nombre,array[i].apellido,array[i].numero,array[i].idAbonado,array[i].isEmpty);
        }
    }
    return retorno;
}
    ...
    if(limite > 0 && array != NULL)
    {
        ...
        if(indice >= 0)
        {
            ...
            if(!getValidString("\nNombre: ","Error","Overflow", nombre,50,2))
            {
                ...
                {
                    if(!getValidInt("\nNumero:","Error",&numero,1000000000,1599999999,2))
                    {
                        retorno = 0;
