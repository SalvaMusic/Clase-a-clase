...
int arrayAlumno_deleteInFile(Alumno* arrayPunterosAlumnos[], int* qtyOcupada, int qtyTotal,int id, char* path)
{
    int retorno = -1;
    int i, auxiliarId, auxiliarIndex;

    FILE* pFile;
    pFile = fopen(path,"w");
    if(pFile != NULL && arrayPunterosAlumnos != NULL && qtyTotal > 0 && *qtyOcupada >= 0 &&  qtyTotal > *qtyOcupada && id >= 0)
    {
        for(i=0;i<*qtyOcupada;i++)
        {
            alumno_getId(arrayPunterosAlumnos[i],&id);
            ...
            {
                auxiliarIndex = i;
                alumno_delete(arrayPunterosAlumnos[i]);
                arrayPunterosAlumnos[i] = arrayPunterosAlumnos[qtyOcupada-1];
                alumno_delete(arrayPunterosAlumnos[qtyOcupada-1]);
            }
        }
    }
    fclose(pFile);
