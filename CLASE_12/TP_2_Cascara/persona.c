...
int persona_mostrar(EPersona* array,int limite)
{
    ...
    if(limite > 0 && array != NULL)
    {
        ...
        for(i=0;i<limite;i++)
        {
            if(!array[i].estado)
            {
                printf("\nNombre: %s \t DNI: %s \t Edad: %d - Id: %d - Estado: %d",array[i].nombre, array[i].dni,array[i].edad,array[i].idPersona,array[i].estado);
                flag=1;
            }
        }
        if (!flag){
...
int persona_alta(EPersona* array,int limite)
{
    ...
    if(limite > 0 && array != NULL)
    {
        ...
        if(indice >= 0)
        {
            ...
            {
                if(!getDni("\nDNI: ","\nError! Ingrese un DNI valido...","\nSe excedio de digitos", dni,11,3))
                {
                     if(!getValidInt("\nEdad: ","\nError! Ingrese una edad valida...", &edad,0,200,2))
                    {
